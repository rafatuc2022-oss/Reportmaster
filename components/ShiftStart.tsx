    ...
    <div className="space-y-6 animate-in fade-in duration-500 pb-20">
      <div className="bg-slate-900/40 border border-slate-800 rounded-2xl p-6 shadow-xl">
        <div className="flex items-center gap-4 mb-8">
          <div className="w-12 h-12 bg-emerald-500/10 text-emerald-500 rounded-xl flex items-center justify-center text-3xl border border-emerald-500/20">📣</div>
          <div>
            <h2 className="text-sm font-black text-white uppercase tracking-tighter leading-none mb-1.5">Boa Jornada</h2>
            <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Relatório de Assunção</p>
          </div>
        </div>
          ...
          {(['A', 'B', 'C', 'D'] as Shift[]).map((shift) => (
            <button
              ...
              className={`py-6 rounded-xl border-2 transition-all flex flex-col items-center justify-center active:scale-95 ${
                selectedShift === shift
                  ? 'border-sky-500 bg-sky-500/10 text-sky-400 shadow-lg'
        ...
        <div className="space-y-3 mb-8">
          <label className="text-[10px] font-black uppercase text-slate-500 tracking-[0.2em] ml-2 block">Visualização da Mensagem</label>
          <textarea
            value={editableMessage}
            onChange={(e) => setEditableMessage(e.target.value)}
            className="w-full h-[550px] p-5 rounded-xl bg-slate-950 border-2 border-slate-800 text-white font-medium text-sm outline-none focus:border-sky-500/30 transition-all resize-none shadow-inner leading-relaxed no-scrollbar"
        ...
        <button
          onClick={handleSend}
          className="w-full h-16 bg-[#25D366] text-slate-950 rounded-xl font-black text-xs uppercase tracking-widest active:scale-95 transition-all shadow-xl flex items-center justify-center gap-3"
        >
